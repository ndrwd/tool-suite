...
import { ShapeMorphCanvas } from "./app/shape-morph/ShapeMorphCanvas";
import { exportShapeMorphPng, exportShapeMorphVideo } from "./app/shape-morph/export";
import "./styles.css";

function handleAction(value: string, state: AppState): void {
  if (value === "export-png") {
    toast.promise(exportShapeMorphPng(state), {
  ...
  if (value === "export-video") {
    ...
    exportShapeMorphVideo(state, (progress) => {
...
createRoot(rootElement).render(
  <React.StrictMode>
    <ToolApp
      ...
      renderer={<ShapeMorphCanvas />}
      schema={appSchema}
      storageKey="shape-morph:state:v1"
    />
  </React.StrictMode>,
);
