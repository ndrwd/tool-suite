...
import { buildMesh, drawMeshGradient } from "./engine";
import { readMeshGradientBackground, readMeshGradientSettings } from "./settings";
...
export async function exportMeshGradientPng(state: AppState): Promise<void> {
  ...
  const settings = readMeshGradientSettings(state.values);

  const canvas = createPngExportCanvas({
    background: readMeshGradientBackground(state.values),
    includeBackground,
    render: ({ context, cssHeight, cssWidth }) => {
      drawMeshGradient(context, {
        blobs: buildMesh(settings, cssWidth, cssHeight),
        blur: settings.blur,
  ...
  if (blob) {
    downloadBlob(blob, `mesh-gradient.${format === "jpg" ? "jpg" : "png"}`);
  }
}
 ...
 */
export async function exportMeshGradientVideo(
  state: AppState,
  ...
  const settings = readMeshGradientSettings(state.values);
  const blobs = buildMesh(settings, cssWidth, cssHeight);
  const background = readMeshGradientBackground(state.values);
  ...
  // how fast individual frames render.
